...
namespace FruitSwipeMatch3Kit
{
    ...
    [AlwaysUpdateSystem]
    public class BoosterResolutionSystem : SystemBase
    {
        ...
        public void ResolveBooster(Entity entity, BoosterData boosterData)
        {
            indexes.Clear();
        ...
        private void ResolveHorizontalBooster(Entity entity)
        {
            ...
            indexes.Add(x + y * width);

            for (var i = 0; i < width; i++)
            {
                var idx = i + y * width;
                if (!EntityManager.HasComponent<HoleSlotData>(levelCreationSystem.TileEntities[idx]) &&
                    !EntityManager.HasComponent<BoosterData>(levelCreationSystem.TileEntities[idx]))
                    indexes.Add(idx);

                if (EntityManager.HasComponent<BoosterData>(levelCreationSystem.TileEntities[idx]) &&
                    !inputSystem.PendingBoosterTiles.Contains(levelCreationSystem.TileEntities[idx]))
                    inputSystem.PendingBoosterTiles.Add(levelCreationSystem.TileEntities[idx]);
            }
        }
