...
namespace FruitSwipeMatch3Kit
{
    ...
    public class LevelCreationSystem : SystemBase, IRestartable
    {
        public int Width;
        public int Height;
        ...
        private void CreateTiles()
        {
            for (var j = 0; j < Height; ++j)
            {
                for (var i = 0; i < Width; ++i)
                {
                    ...
                    if (levelData.Tiles[idx].TileType == TileType.Color ||
                        levelData.Tiles[idx].TileType == TileType.RandomColor)
                    {
                        EntityManager.AddComponentData(tileEntity, new TileData
                        {
                            Type = tileGo.GetComponent<TileDataProxy>().Type
                        });
                    }
                    else if (levelData.Tiles[idx].TileType == TileType.Blocker)
                    {
                        EntityManager.AddComponentData(tileEntity, new BlockerData
                        {
                            Type = tileGo.GetComponent<BlockerDataProxy>().Type
                        });
                    }
                    else if (levelData.Tiles[idx].TileType == TileType.Collectible)
                    {
                        EntityManager.AddComponentData(tileEntity, new CollectibleData
                        {
                            Type = tileGo.GetComponent<CollectibleDataProxy>().Type
                        });
                    }

                    EntityManager.AddComponentData(tileEntity, new TilePosition { X = i, Y = j });
