...
namespace FruitSwipeMatch3Kit
{
    ...
    public class AnimateGravitySystem : SystemBase
    {
        ...
        protected override void OnUpdate()
        {
            ...
            if (hasGravity)
            {
                ...
                seq.AppendCallback(() =>
                {
                    ...
                    {
                        var boosterResolutionSystem = World.GetExistingSystem<BoosterResolutionSystem>();
                        var tile = inputSystem.PendingBoosterTiles[0];
                        if (EntityManager.Exists(tile))
                        {
                            boosterResolutionSystem.ResolveBooster(tile, EntityManager.GetComponentData<BoosterData>(tile));
                        }
                        else
                        {
                            inputSystem.PendingBoosterTiles.Clear();
                        }
                    }
                    else
                    {
