    ...
    public class PlayerInputSystem : SystemBase
    {
        ...
        private void OnMouseUp()
        {
            ...
            if (selectedTiles.Count >= GameplayConstants.NumTilesNeededForMatch)
            {
                ...
                if (tilesToDestroy.Count == 0 && selectedBooster != null)
                {
                    var boosterResolutionSystem = World.GetExistingSystem<BoosterResolutionSystem>();
                    boosterResolutionSystem.ResolveBooster(PendingBoosterTiles[0], EntityManager.GetComponentData<BoosterData>(PendingBoosterTiles[0]));
                }
                else
                {
